...
async function changeTelp(UUID = "", telp = '') {
  try {
    if (!isMobilePhone(telp, 'id-ID')) { throw Error('invalid telp!') };
    ...
    return { status: true, message: "sukses ganti no telp" }
  } catch (error) {
    return { status: false, message: "gagal ganti no telp", error }
  }
}
...
async function main() {
  try {
    console.log(await changeTelp('7838a342-05b1-447a-8c5d-5ec109461067', '081229439547'));
  } catch (error) {
    console.log(error);
  }
}
