...
async function suka(dom) {
  simpan.on = false; bagi.on = false; komentar.on = false;
  if (suka.on === true) {
    suka.on = false;
...
async function simpan(dom) {
  suka.on = false; bagi.on = false; komentar.on = false;
  if (simpan.on === true) {
    simpan.on = false;
...
async function bagi(dom) {
  suka.on = false; simpan.on = false; komentar.on = false;
  if (bagi.on === true) {
    bagi.on = false;
...
async function komentar(dom) {
  suka.on = false; simpan.on = false; bagi.on = false;
  if (komentar.on === true) {
    komentar.on = false;
    return document.getElementById('comments-container').style.display = 'none';
  }
  komentar.on = true;
  document.querySelectorAll('.interactions-container').forEach(a => {
